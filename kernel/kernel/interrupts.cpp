...
{
    printf("isr  0 works\r");
}

extern "C" void isr_handler()
{
    printf("isr gen works\r");
}
...
extern "C" void init_table()
{
    ...
    for(int x = 0; x < 48; x++)
    {
        if(x < 6)
        {
            switch(x)
            {
                case 0: irqAddress = (uint64_t)isr0;break;
                case 1: irqAddress = (uint64_t)isr1;break;
                case 2: irqAddress = (uint64_t)isr2;break;
                case 3: irqAddress = (uint64_t)isr3;break;
                case 4: irqAddress = (uint64_t)isr4;break;
                case 5: irqAddress = (uint64_t)isr5;break;
            }
            
        }
        else if(x < 32)
        {
            irqAddress = (uint64_t)isr1;
        }
        else
        {
            irqAddress = (uint64_t)irq0;
        }
        
        idt[x].offset_low = irqAddress & 0xFFFF;
        idt[x].segment_selector = 0x8; //interrupt gate
        idt[0].zero = 0;
        idt[x].offset_middle = (irqAddress&0xFFFF0000) >> 16;
