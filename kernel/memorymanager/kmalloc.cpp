...
{
	int i = 0;
	struct boundary_tag *tag = nullptr;
	...
	printf("Memory in used (malloc'ed): %i\n", l_inuse );
		...
		for ( i = 0; i < MAXEXP; i++ )
		{
			...
			while ( tag != nullptr )
			{
				if ( tag->split_left  != nullptr  ) printf("*");
				printf("%i", tag->real_size );
				if ( tag->split_right != nullptr  ) printf("*");
...
static struct boundary_tag* allocate_new_tag( unsigned int size )
{
	...
	struct boundary_tag *tag;
		...
		if ( tag == nullptr ) return nullptr;	// uh oh, we ran out of memory.
...
void*   realloc(void *p, size_t size)
{
	...
	if ( liballoc_lock != nullptr ) liballoc_lock();		// lockit
		tag = (struct boundary_tag*)((unsigned int)p - sizeof( struct boundary_tag ));
		real_size = tag->size;
	if ( liballoc_unlock != nullptr ) liballoc_unlock();
